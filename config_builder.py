...
def build_nomad_config(is_server, dc, aj):
    conf = 'client_addr = "0.0.0.0"\ndata_dir    = "/opt/nomad"'
    build_hcl_config(conf, 'server = {}'.format(str(is_server).lower()))
    build_hcl_config(conf, 'datacenter = {}'.format(str(dc).lower()))
    build_hcl_config(conf, 'retry_join = [{}]'.format(str(aj)))
    return conf
